...
if (TFCache::start_cache('image', self::$post_id, array('ID' => $module_ID))):
	...
	if ($this->is_img_php_disabled()) {
		...
		$image = '<img src="' . esc_url($url_image) . '" alt="' . esc_attr($image_alt) . '" width="' . esc_attr($width_image) . '" height="' . esc_attr($height_image) . '" class="'. $class .'">';
	} else {
	...
	<div id="<?php echo esc_attr($module_ID); ?>" class="<?php echo esc_attr($container_class); ?>">
		...
		<div class="image-wrap">
			<?php if (!empty($link_image)): ?>
				...
				if ($lightbox) : echo 'class="lightbox-builder lightbox"';
				endif;
				?> <?php
